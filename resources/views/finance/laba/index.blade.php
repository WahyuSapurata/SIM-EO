                            ...
                            <div class="py-5 table-responsive">
                                <table id="kt_table_data"
                                    ...
                                    {{-- <tfoot class="bg-primary">
                                        <tr class="fw-bolder fs-6 text-gray-800">
                                            <td style="text-align: left !important;" colspan="6">TOTAL
                                            </td>
                                            <td style="text-align: left !important;" colspan="2" id="total">
                                                Rp 0
                                            </td>
                                        </tr>
                                    </tfoot> --}}
        ...
        <div class="card w-100">
            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        <label class="form-label">Budget Client</label>
                        <input type="text" id="budget_client" class="form-control" name="budget_client">
                        <small class="text-danger budget_client_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Real Cost</label>
                        <input type="text" id="real_cost" class="form-control" name="real_cost">
                        <small class="text-danger real_cost_error"></small>
                    </div>
                    ...
                    <div class="mb-10">
                        <label class="form-label">Operasional Kantor</label>
                        <input type="text" id="operasional_kantor" class="form-control" name="operasional_kantor">
                        <small class="text-danger operasional_kantor_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $('#budget_client').on('input', function() {
            let value = $(this).val();
            if (value !== "") {
        ...
        $('#operasional_kantor').on('input', function() {
            let value = $(this).val();
            if (value !== "") {
                value = numeral(value).format('0,0'); // Format to rupiah
                $(this).val('Rp ' + value);
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Laporan Laba Bersih');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/data-laporan/add-laporan-laba', 'Tambah',
                    'Laporan Laba Bersih',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/data-laporan/update-laporan-laba/' + uuid, 'Update',
                    'Laporan Laba Bersih', 'POST');
            }
        });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/data-laporan/show-laporan-laba/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Laporan Laba Bersih', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/data-laporan/delete-laporan-laba/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/data-laporan/get-laporan-laba',
                columns: [{
                    data: null,
                    ...
                    className: 'text-center',
                    render: function(data, type, row, meta) {
                        var budget = parseFloat(row.budget_client);
                        var cost = parseFloat(row.real_cost);
                        var ops = parseFloat(row.operasional_kantor);
                        var pph = parseFloat(row.pph);
                        const value = numeral(budget -
                            cost - pph - ops).format(
                            '0,0'); // Format to rupiah
                        return 'Rp ' + value;
                    }
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        if (full.sisa_tagihan) {
                            return '<div class="btn btn-success px-2 py-1">Setujui</div>';
                        } else {
                            return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Laporan Laba Bersih" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
